...
class BeadGeometryObserver:
    ...
    def _extract_layer_height(self, valid_scans, current_z_shift):
        ...
        sort_idx = np.lexsort((arr_x, arr_sg))
        arr_x = arr_x[sort_idx]
        arr_y = arr_y[sort_idx]
        arr_h = arr_h[sort_idx]
        arr_sg = arr_sg[sort_idx]

        splits = np.nonzero(arr_sg[1:] != arr_sg[:-1])[0] + 1
        h_groups = np.split(arr_h, splits)

        win = int(self.p['MEDIAN_WINDOW'])
        thresh = self.p['MEDIAN_THRESHOLD']
        ...
        mask_median = np.concatenate(clean_masks)

        valid_x = arr_x[mask_median]
        valid_y = arr_y[mask_median]
        valid_h = arr_h[mask_median]
        valid_sg = arr_sg[mask_median]
